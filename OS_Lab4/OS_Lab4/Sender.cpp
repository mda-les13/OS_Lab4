...
int main(int argc, char* argv[]) {
    ...
    HANDLE hSenderReadyEvent = OpenEventW(EVENT_MODIFY_STATE, FALSE, eventName.c_str());
    if (hSenderReadyEvent == NULL) {
        std::cerr << "Error opening sender ready event!" << std::endl;
        return 1;
    }

    HANDLE hMessageAvailableEvent = OpenEventW(EVENT_MODIFY_STATE, FALSE, L"MessageAvailableEvent");
    ...
    HANDLE hFileMutex = OpenMutexW(SYNCHRONIZE, FALSE, L"FileMutex");
    if (hFileMutex == NULL) {
